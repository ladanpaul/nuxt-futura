<template>
  <div class="relative flex flex-col w-[160px] bg-black rounded-xl h-[60px]">
    <div
      class="absolute left-0 right-0 top-0 bottom-0 flex items-center h-[60px] bg-pastelGreen transition-width duration-1000 ease-linear rounded-xl -z-1"
      :style="{ width: progressBarWidth }"
    ></div>
    <span
      class="flex items-center m-auto text-2xl font-bold text-center w-[64px] text-white"
    >
      {{ formattedTime }}
    </span>
  </div>
