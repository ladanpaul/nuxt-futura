<template>
  <div class="relative">
    <!-- :class="[
      'flex relative h-[46px] border rounded-lg',
      {
        'border-linkWater': theme === 'white',
        'border-transparent focus:border-cornflowerBlue bg-waikawaGrey':
          theme === 'dark',
      },
    ]" -->
    <span
    ...
    <input
      ...
      :value="value"
      :class="[
        'w-full border border-transparent outline-none text-xs h-[46px]  py-1 pl-4 pr-3 rounded-lg',
        {
      ...
      @input="onInput($event)"
    />
    <div class="absolue right-3 top-1/2 transform -translate-y-1/2">
      <slot name="icon" />
    </div>
  </div>
</template>
