<template>
  <div class="relative text-raven">
    ...
    <div
      v-if="isOpen"
      class="absolute top-full flex flex-col h-[200px] w-full z-1 bg-white overflow-auto rounded-bl-lg rounded-br-lg border border-linkWater"
    >
      <div
        v-for="(item, idx) in items"
        :key="idx"
        class="flex items-center w-full px-4 min-h-32px text-xs cursor-pointer border-b border-linkWater last:border-b-0 last:rounded-bl-lg last:rounded-br-lg"
        @click="select(item)"
      >
        <span>{{ item }}</span>
