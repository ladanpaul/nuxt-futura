  ...
  <div class="relative text-raven">
    ...
    >
      <div
        ...
        :key="idx"
        :class="[
          'flex items-center w-full px-4 min-h-32px text-xs cursor-pointer border-b border-linkWater last:border-b-0 last:rounded-bl-lg last:rounded-br-lg',
          { 'bg-pastelGreen': selectedValue === item },
        ]"
        @click="select(item)"
      >
        <span>{{ item }}</span>
